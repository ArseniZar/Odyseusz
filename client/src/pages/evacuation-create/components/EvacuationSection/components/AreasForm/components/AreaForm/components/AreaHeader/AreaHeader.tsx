...
import type { AreaHeaderProps } from "./AreaHeader.types";
import { IconButton } from "@/components/IconButton";


export const AreaHeader = ({ title, areaNumber, isOpen ,className, onDelete, onToggle,icons }: AreaHeaderProps) : JSX.Element => {
  return(
  <div className={`flex flex-row  items-center justify-between ${className}`}> 
    <button onClick={onToggle} className="flex-1  px-6 py-4 flex flex-row">
      <h3 className="text-xl font-light">{title} {areaNumber}</h3>
    </button>
